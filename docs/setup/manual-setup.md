...
1. **Run the EXPORT pipeline** to export your solution(s) from the Dev environment
2. **Run the BUILD pipeline** to build the managed solution(s)
3. **Run the DEPLOY pipeline** to deploy to your target environments
4. Configure environment-specific values in the variable groups (see [environment-variable-group.md](environment-variable-group.md))
...
### Pipeline fails with "Resource authorization failed"

Grant the pipeline access to the required resources (repositories, service connections, variable groups, environments) as described in [Pipeline Permissions](#9-pipeline-permissions).

### Build Service lacks repository permissions

Ensure the Build Service has Contribute permission on your main repository as described in [Grant Build Service Permissions](#33-grant-build-service-permissions).

### Service connection authentication fails

- Verify the Application ID and Client Secret are correct
- Ensure the App Registration has a Service Principal created in Entra ID
- Verify the client secret has not expired
- Check that the application user exists in the Dataverse environment with System Administrator role

### Pipeline cannot find variable group

- Verify the variable group name matches exactly (case-sensitive): `Environment-{EnvironmentName}`
- Ensure the pipeline has permission to access the variable group
